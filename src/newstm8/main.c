/*-------------------------------------------------------------------------
  main.c - newSTM8 specific definitions.
...
static int
newstm8_do_pragma (int id, const char *name, const char *cp)
{
...
static struct pragma_s newstm8_pragma_tbl[] = {
  {"codeseg", P_CODESEG, 0, newstm8_do_pragma},
  {"constseg", P_CONSTSEG, 0, newstm8_do_pragma},
  {NULL, 0, 0, NULL},
};

static int
newstm8_process_pragma (const char *s)
{
  return process_pragma_tbl (newstm8_pragma_tbl, s);
}

static char newstm8_defaultRules[] = {
#include "peeph.rul"
  ""
};


static char *newstm8_keywords[] = {
  "at",
  "critical",
  "interrupt",
  "trap",
...
static void
newstm8_genAssemblerEnd (FILE *of)
{
...
extern void newstm8_init_asmops (void);
...
static void
newstm8_reset_regparm (struct sym_link *ftype)
{
  _G.regparam.n = 0;
...
static int
newstm8_reg_parm (sym_link *l, bool reentrant)
{
  bool is_regarg = newstm8IsRegArg(_G.regparam.ftype, ++_G.regparam.n, 0);

  return (is_regarg ? _G.regparam.n : 0);
}

static bool
newstm8_parseOptions (int *pargc, char **argv, int *i)
{
...
static void
newstm8_setDefaultOptions (void)
{
  options.nopeep = 0;
  options.stackAuto = 1;
...
static const char *
newstm8_getRegName (const struct reg_info *reg)
{
  if (reg)
    return reg->name;
  return "err";
...
static void
newstm8_genExtraArea (FILE *of, bool hasMain)
{
...
static void
newstm8_genInitStartup (FILE *of)
{
  ...
  fprintf (of, "; newstm8_genXINIT() start\n");
  ...
  fprintf (of, "00004$:\n");
  fprintf (of, "; newstm8_genXINIT() end\n");
}

#define newSTM8_INTERRUPTS_COUNT 30

int
newstm8_genIVT(struct dbuf_s * oBuf, symbol ** intTable, int intCount)
{
  int i;
  dbuf_tprintf (oBuf, "\tint s_GSINIT ; reset\n");

  if(intCount > newSTM8_INTERRUPTS_COUNT)
    {
      werror(E_INT_BAD_INTNO, intCount - 1);
      intCount = newSTM8_INTERRUPTS_COUNT;
    }
...
/*----------------------------------------------------------------------*/
/* newstm8_dwarfRegNum - return the DWARF register number for a register.  */
/*----------------------------------------------------------------------*/
static int
newstm8_dwarfRegNum (const struct reg_info *reg)
{
  return reg->rIdx;
}
...
{
  "sdldnewstm8", "-nf", "\"$1\"", NULL
};

/* $3 is replaced by assembler.debug_opts resp. port->assembler.plain_opts */
static const char *newstm8AsmCmd[] =
{
  "sdasnewstm8", "$l", "$3", "\"$1.asm\"", NULL
};

static const char *const _libs_newstm8[] = { "newstm8", NULL, };
...
PORT newstm8_port =
{
  ...
  {
    glue,
    TRUE,                       /* We want newstm8_genIVT to be triggered */
  ...
  {                             /* Peephole optimizer */
    newstm8_defaultRules,
    newstm8instructionSize,
    NULL,
    NULL,
    NULL,
    newstm8notUsed,
  ...
  "_",
  newstm8_init,
  newstm8_parseOptions,
  newstm8_options,
  NULL,
  newstm8_finaliseOptions,
  newstm8_setDefaultOptions,
  newstm8_assignRegisters,
  newstm8_getRegName,
  0,
  NULL,
  newstm8_keywords,
  NULL,
  newstm8_genAssemblerEnd,
  newstm8_genIVT,
  0,                            /* no genXINIT code */
  newstm8_genInitStartup,          /* genInitStartup */
  newstm8_reset_regparm,
  newstm8_reg_parm,
  newstm8_process_pragma,          /* process_pragma */
