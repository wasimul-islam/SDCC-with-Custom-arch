...
/*-----------------------------------------------------------------*/
void
newstm8SpillThis (symbol *sym, bool force_spill)
{
  int i;

  D (D_ALLOC, ("newstm8SpillThis: spilling %p (%s)\n", sym, sym->name));

  /* if this is rematerializable or has a spillLocation
     we are okay, else we need to create a spillLocation
     for it */
  if (!(sym->remat || sym->usl.spillLoc) || (sym->usl.spillLoc && !sym->usl.spillLoc->onStack)) // newstm8 port currently only supports on-stack spill locations in code generation.
    createStackSpil (sym);
...
packRegsForAssign (iCode *ic, eBBlock *ebp)
{
  ...
  /* Otherwise, for now eliminate 8-bit temporary variables only.
     The newSTM8 instructions operating directly on memory
...
serialRegMark (eBBlock ** ebbs, int count)
{
  ...
  newstm8_call_stack_size = 2; // Saving of register to stack temporarily.
  ...
  for (i = 0; i < count; i++)
    {
      ...
      for (ic = ebbs[i]->sch; ic; ic = ic->next)
        {
          if ((ic->op == CALL || ic->op == PCALL) && ic->parmBytes + 5 > newstm8_call_stack_size)
            {
              sym_link *dtype = operandType (IC_LEFT (ic));
              sym_link *ftype = IS_FUNCPTR (dtype) ? dtype->next : dtype;

              /* 5 for saving all registers at call site + 2 for big return value */
              newstm8_call_stack_size = ic->parmBytes + 5 + 2 * (getSize (ftype->next) > 4);
            }
...
verifyRegsAssigned (operand * op, iCode * ic)
{
  ...
  newstm8SpillThis (sym, FALSE);
}

void
newstm8RegFix (eBBlock ** ebbs, int count)
{
  int i;
...
/*-----------------------------------------------------------------*/
void
newstm8_assignRegisters (ebbIndex *ebbi)
{
  ...
  newstm8_extend_stack = newstm8_call_stack_size > 255;

  /* Invoke optimal register allocator */
  ic = newstm8_ralloc2_cc (ebbi);

  /* redo the offsets for stacked automatic variables */
  if (currFunc && !newstm8_extend_stack && currFunc->stack + newstm8_call_stack_size > 255)
    {
      _G.slocNum = 0;
