...
extern bool newstm8_regs_used_as_parms_in_calls_from_current_function[YH_IDX + 1];
extern bool newstm8_regs_used_as_parms_in_pcalls_from_current_function[YH_IDX + 1];
...
/*-----------------------------------------------------------------*/
/* newstm8InstIsRegToReg - Checks if 'line' is a reg to reg move      */
/* isword == FALSE : Look for registers a, xl, xh, yl & yh         */
/* isword == TRUE  : Look for registers x, y & sp                  */
/*-----------------------------------------------------------------*/
static bool
newstm8InstIsRegToReg(const char *line, bool isword)
{
  int regNumber = 0;
...
int
newstm8instructionSize(lineNode *pl)
...
static bool argCont(const char *arg, char what)
{
  ...
  if (arg[0] == '_' && what == 'a') // The newSTM8 has no a-relative addressing modes.
    return FALSE;
...
static bool
newstm8MightReadFlag(const lineNode *pl, const char *what)
{
...
static bool
newstm8MightBeParmInCallFromCurrentFunction(const char *what)
{
  if (!strcmp(what, "a"))
    return newstm8_regs_used_as_parms_in_calls_from_current_function[A_IDX];
    
  if ((!strcmp(what, "x") || !strcmp(what, "xl")) && newstm8_regs_used_as_parms_in_calls_from_current_function[XL_IDX])
    return true;
  if ((!strcmp(what, "x") || !strcmp(what, "xh")) && newstm8_regs_used_as_parms_in_calls_from_current_function[XH_IDX])
    return true;

  if ((!strcmp(what, "y") || !strcmp(what, "yl")) && newstm8_regs_used_as_parms_in_calls_from_current_function[YL_IDX])
    return true;
  if ((!strcmp(what, "yx") || !strcmp(what, "yh")) && newstm8_regs_used_as_parms_in_calls_from_current_function[YH_IDX])
    return true;
...
static bool
newstm8MightBeParmInPCallFromCurrentFunction(const char *what)
{
  if (!strcmp(what, "a"))
    return newstm8_regs_used_as_parms_in_pcalls_from_current_function[A_IDX];

  if ((!strcmp(what, "x") || !strcmp(what, "xl")) && newstm8_regs_used_as_parms_in_pcalls_from_current_function[XL_IDX])
    return true;
  if ((!strcmp(what, "x") || !strcmp(what, "xh")) && newstm8_regs_used_as_parms_in_pcalls_from_current_function[XH_IDX])
    return true;

  if ((!strcmp(what, "y") || !strcmp(what, "yl")) && newstm8_regs_used_as_parms_in_pcalls_from_current_function[YL_IDX])
    return true;
  if ((!strcmp(what, "yx") || !strcmp(what, "yh")) && newstm8_regs_used_as_parms_in_pcalls_from_current_function[YH_IDX])
    return true;
...
static bool
newstm8MightRead(const lineNode *pl, const char *what)
{
  char extra = 0;
  ...
  if (!strcmp (what, "xl") || !strcmp (what, "xh"))
    ...
    return newstm8MightReadFlag(pl, what);
  ...
  if (ISINST (pl->line, "call") || ISINST (pl->line, "callr") || ISINST (pl->line, "callf"))
    {
      ...
      if (f)
        return newstm8IsParmInCall(f->type, what);
      else // Fallback needed for calls through function pointers and for calls to literal addresses.
        return (newstm8MightBeParmInCallFromCurrentFunction(what) || newstm8MightBeParmInPCallFromCurrentFunction(what));
    }

  if(ISINST(pl->line, "ret")) // IAR calling convention uses ret for some calls via pointers
    return(newstm8IsReturned(what) || newstm8MightBeParmInPCallFromCurrentFunction(what));
...
static bool
newstm8UncondJump(const lineNode *pl)
{
  return (ISINST(pl->line, "jp") || ISINST(pl->line, "jra") || ISINST(pl->line, "jrt") || ISINST(pl->line, "jpf"));
}

static bool
newstm8CondJump(const lineNode *pl)
{
  return (!newstm8UncondJump(pl) && STARTSINST(pl->line, "jr") ||
    ISINST(pl->line, "btjt") || ISINST(pl->line, "btjf"));
}

static bool
newstm8SurelyWritesFlag(const lineNode *pl, const char *what)
{
  ...
  if (!strcmp (what, "n") || !strcmp (what, "z"))
    {
      ...
      if (ISINST (pl->line, "ld"))
        return !newstm8InstIsRegToReg(pl->line, false);
      if (ISINST (pl->line, "ldw"))
        return !newstm8InstIsRegToReg(pl->line, true);
...
static bool
newstm8SurelyWrites(const lineNode *pl, const char *what)
{
  char extra = 0;
  if (!strcmp (what, "xl") || !strcmp (what, "xh"))
    extra = 'x';
    ...
    return (newstm8SurelyWritesFlag (pl, what));
...
static bool
newstm8SurelyReturns(const lineNode *pl)
{
  ...
  for (; *pl; *pl = (*pl)->next)
    {
      ...
      if(newstm8SurelyWrites(*pl, what))
        {
          D(("S4O_WR_OP\n"));
          return S4O_WR_OP;
        }

      if(newstm8UncondJump(*pl))
        {
          *pl = findLabel (*pl);
            if (!*pl)
              {
      ...
      /* Don't need to check for de, hl since newstm8MightRead() does that */
      if(newstm8SurelyReturns(*pl))
        {
          D(("S4O_TERM\n"));
          return S4O_TERM;
        }
...
bool
newstm8notUsed (const char *what, lineNode *endPl, lineNode *head)
{
  lineNode *pl;
  if(strcmp(what, "x") == 0)
    return(newstm8notUsed("xl", endPl, head) && newstm8notUsed("xh", endPl, head));
  else if(strcmp(what, "y") == 0)
    return(newstm8notUsed("yl", endPl, head) && newstm8notUsed("yh", endPl, head));
...
bool
newstm8notUsedFrom (const char *what, const char *label, lineNode *head)
{
  lineNode *cpl;

  for (cpl = head; cpl; cpl = cpl->next)
    if (cpl->isLabel && !strncmp (label, cpl->line, strlen(label)))
      return (newstm8notUsed (what, cpl, head));
...
bool
newstm8canAssign (const char *op1, const char *op2, const char *exotic)
{
