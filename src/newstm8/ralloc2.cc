...
extern "C"
{
  #include "ralloc.h"
  #include "gen.h"
  float drynewSTM8iCode (iCode *ic);
  bool newstm8_assignment_optimal;
  long int newstm8_call_stack_size;
  bool newstm8_extend_stack;
}
...
static bool Ainst_ok(const assignment &a, unsigned short int i, const G_t &G, const I_t &I)
{
  ...
  if(ic->op == IPUSH)
    {
      if (ia.registers[REG_XL][1] < 0 || ia.registers[REG_YL][1] < 0 && !newstm8_extend_stack)
  ...
  for(boost::tie(oi, oi_end) = G[i].operands.equal_range(OP_SYMBOL_CONST(o)->key); oi != oi_end; ++oi)
    {
      ...
      if(a.global[v] >= 0)
        { 
          sym->regs[I[v].byte] = newstm8_regs + a.global[v];   
          sym->nRegs = I[v].size;
        }
      else
        {
...
{
  // newstm8 code generation is very flexible, and can handle nearly anything (including variables where some bytes are spilt, while others are not).
  ...
  for(unsigned int v = 0; v < boost::num_vertices(I); v++)
    {
      ...
      if(winner.global[v] >= 0)
        sym->regs[I[v].byte] = newstm8_regs + winner.global[v];   
      else
        {
          sym->regs[I[v].byte] = 0;
          spilt = true;
...
iCode *newstm8_ralloc2_cc(ebbIndex *ebbi)
{
  ...
  newstm8_assignment_optimal = !tree_dec_ralloc(tree_decomposition, control_flow_graph, conflict_graph, stack_conflict_graph);

  newstm8RegFix (ebbs, count);
